...
<body>
    <div class="container">
        ...
        <div style="margin: 2em 0 1em 0;">
            <label style="font-weight:600;display:block;margin-bottom:0.5em;">Pilih Metode Pembayaran:</label>
            <div style="display:flex;flex-direction:column;gap:1em;">
                <label><input type="radio" name="payment_method" value="qris" checked> QRIS</label>
                <label><input type="radio" name="payment_method" value="dana"> eWallet DANA</label>
                <label><input type="radio" name="payment_method" value="va_bri"> Virtual Account BRI</label>
            </div>
        </div>
    ...
    <script type="module">
        ...
        document.getElementById('confirm-order-btn').addEventListener('click', function() {
            ...
            fetch('{{ route("order.store") }}', {
                ...
                body: JSON.stringify({table_number: '{{ $tableNumber }}', cart: JSON.stringify(cart), payment_method: paymentMethod})
            })
            .then(res => res.json())
            .then(data => {
                if (data.success && data.order_id) {
                    ...
                    if (paymentMethod === 'qris') {
                        ...
                        window.location.href = '/dana-payment';
                    } else if (paymentMethod === 'va_bri') {
                        window.location.href = '/bri-payment';
                    } else {
