...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Detail Pesanan #{{ $order->id }}</title>
...
<body class="font-sans antialiased bg-gray-100">
    <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-100" @keydown.escape.window="sidebarOpen = false">
        ...
        >
            ...
            <nav class="mt-4">
                {{-- Link Dashboard --}}
                <a href="{{ route('dashboard') }}" class="flex items-center px-6 py-3 text-white bg-gray-700">
                ...
                <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
                    ...
                    <div class="bg-white rounded-xl shadow-xl p-8 border border-gray-200">
                        ...
                        <div class="flex justify-between items-center mb-7 border-b-2 pb-4 border-blue-300">
                            ...
                            <a href="{{ route('dashboard') }}" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-semibold rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                &larr; Kembali ke Dashboard
                            </a>
                        </div>
                        
                        {{-- Info Pesanan --}}
                        <dl class="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 mb-8">
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Order ID</dt>
                                <dd class="mt-1 text-lg font-semibold text-gray-900">#{{ $order->id }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Nomor Meja</dt>
                                <dd class="mt-1 text-lg font-semibold text-gray-900">{{ $order->table_number }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Waktu Pesan</dt>
                                <dd class="mt-1 text-lg font-semibold text-gray-900">{{ $order->created_at->format('d M Y, H:i') }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Status</dt>
                                <dd class="mt-1">
                                    <span class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full {{ $order->status->color() }} text-gray-800 shadow-sm">
                                        {{ $order->status->label() }}
                                    </span>
                                </dd>
                            </div>
                        ...
                        {{-- Tabel Item Pesanan --}}
                        <div class="mt-8">
                            <h4 class="text-xl font-bold mb-4 text-gray-800">Item Pesanan</h4>
                             <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-300">
                                    <thead class="bg-blue-50">
                                        <tr>
                                            <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Produk</th>
                                            <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Catatan</th>
                                            <th class="py-4 px-6 text-center text-xs font-bold text-blue-700 uppercase tracking-wider">Jumlah</th>
                                            <th class="py-4 px-6 text-right text-xs font-bold text-blue-700 uppercase tracking-wider">Harga Satuan</th>
                                            <th class="py-4 px-6 text-right text-xs font-bold text-blue-700 uppercase tracking-wider">Subtotal</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($order->items as $item)
                                            <tr class="hover:bg-blue-50 transition-colors duration-200">
                                                <td class="py-4 px-6 whitespace-nowrap font-medium text-gray-800">{{ $item->product->name }}</td>
                                                <td class="py-4 px-6 text-sm text-gray-600 italic">{{ $item->notes ?? '-' }}</td>
                                                <td class="py-4 px-6 text-center text-sm text-gray-700">x{{ $item->quantity }}</td>
                                                <td class="py-4 px-6 text-right text-sm text-gray-700">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
                                                <td class="py-4 px-6 text-right text-sm font-semibold text-gray-800">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</td>
                                            </tr>
                                        @endforeach
                                        <tr class="font-bold bg-blue-50">
                                            <td colspan="4" class="py-4 px-6 text-right text-blue-800 uppercase text-sm">Total Harga</td>
                                            <td class="py-4 px-6 text-right text-blue-800 text-lg">Rp {{ number_format($order->total_price, 0, ',', '.') }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        @if($order->status != \App\Enums\OrderStatus::COMPLETED && $order->status != \App\Enums\OrderStatus::CANCELLED)
                            <div class="mt-8 pt-6 border-t-2 border-gray-200 flex justify-end items-center space-x-4">
                                <span class="text-sm font-semibold text-gray-600">Ubah Status Pesanan:</span>
                                ...
                                <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                                    ...
                                    <button type="submit" class="inline-flex items-center justify-center px-5 py-2.5 border border-transparent text-sm font-semibold rounded-lg shadow-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                                        Tandai Selesai
                                    </button>
                                </form>
                            </div>
