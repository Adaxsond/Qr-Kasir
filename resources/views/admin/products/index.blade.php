...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Manajemen Produk</title>
                ...
                <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
                    ...
                    <div class="bg-white rounded-xl shadow-xl p-8 border border-gray-200">
                        ...
                        <div class="flex justify-between items-center mb-7 border-b-2 pb-4 border-blue-300">
                            ...
                            <a href="{{ route('admin.products.create') }}" class="inline-flex items-center justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-300">
                                <thead class="bg-blue-50">
                                    <tr>
                                        <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Gambar</th>
                                        <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Nama Produk</th>
                                        <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Kategori</th>
                                        <th class="py-4 px-6 text-left text-xs font-bold text-blue-700 uppercase tracking-wider">Harga</th>
                                    ...
                                    @forelse ($products as $product)
                                        <tr class="hover:bg-blue-50 transition-colors duration-200">
                                            <td class="py-4 px-6 whitespace-nowrap">
                                                <img src="{{ $product->image_path ? asset('storage/' . $product->image_path) : 'https://via.placeholder.com/80x80' }}" 
                                                     alt="{{ $product->name }}" 
                                                     class="w-16 h-16 object-cover rounded-md shadow-sm">
                                            </td>
                                            <td class="py-4 px-6 whitespace-nowrap text-sm font-medium text-gray-800">{{ $product->name }}</td>
                                            <td class="py-4 px-6 whitespace-nowrap text-sm text-gray-700">{{ $product->category->name ?? 'Tanpa Kategori' }}</td>
                                            <td class="py-4 px-6 whitespace-nowrap text-sm text-gray-700">Rp {{ number_format($product->price, 0, ',', '.') }}</td>
                                            <td class="py-4 px-6 text-right whitespace-nowrap text-sm space-x-2">
                                                <a href="{{ route('admin.products.edit', $product) }}" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                    Edit
                                                </a>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="py-8 px-6 text-center text-gray-500 text-lg italic">
                                                ...
                                                Tidak ada produk ditemukan. Silakan tambah produk baru.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
